...
function updateCompletionUI(programName) {
  ...
  document
    .querySelectorAll(`.program-card[data-name="${programName}"]`)
    .forEach((card) => {
      ...
      if (completionBtn) {
        ...
        completionBtn.setAttribute(
          "aria-label",
          isCompleted ? "Mark as not completed" : "Mark as completed",
        );
        completionBtn.setAttribute(
          "onClick",
          isCompleted ? null : "doConfetti()",
        );
  ...
  if (modal.classList.contains("active")) {
    ...
    if (modalTitle === programName) {
      const modalCompletionBtn = modal.querySelector(
      ...
      const modalCompletionBadge = modal.querySelector(
...
function getParticipantTotal(fields) {
  const totalKey = Object.keys(fields || {}).find(
    (key) => key.startsWith("Unweighted") && key.endsWith("Total"),
  );
...
function createProgramCard(program) {
  ...
  const accelerateClass =
    program.name === "Accelerate" ? "accelerate-card" : "";
  ...
  const wackyFilesClass =
    program.name === "Wacky Files" ? "wacky-files-card" : "";
  const flavortownClass =
    program.name === "Flavortown" ? "flavortown-card" : "";
  ...
  const hackanomousClass =
    program.name === "Hackanomous" ? "hackanomous-card" : "";
  ...
  const isNew =
    program.opens &&
    new Date() - new Date(program.opens) < 7 * 24 * 60 * 60 * 1000;
  const encodedProgram = encodeURIComponent(JSON.stringify(program));
  const polygonClass = program.name === "Polygon" ? "polygon-card" : "";
  const treasureHuntClass =
    program.name === "Treasure Hunt" ? "treasure-hunt-card" : "";
  ...
  const flavortownFooter =
    program.name === "Flavortown"
      ? `
        <img src="logos/flavorfooter.avif" alt="" class="flavortown-footer">
    `
      : "";

  const jusstudyAssets =
    program.name === "Jus'STUDY"
      ? `
  ...
  const hctgLogo =
    program.name == "Hack Club: The Game"
      ? `
  ...
  const raspapiPi =
    program.name == "RaspAPI"
      ? `<img src="https://raspapi.hackclub.com/rpizero-topdown.png" alt="" class="raspapi-pi" aria-hidden="true">`
      : "";
  const beestSticker =
    program.name == "Beest"
      ? `<img src="logos/beest-sticker.webp" alt="Beest sticker" class="beest-sticker" loading="lazy">`
      : "";
  const forgeSticker =
    program.name == "Forge"
      ? `<img src="logos/sticker_forge.svg" alt="Forge sticker" class="forge-sticker" loading="lazy">`
      : "";

  const alchemize =
    program.name === "Alchemize"
      ? `<img src="https://alchemize-ysws.vercel.app/Alchemist.webp" alt="Alchemize Logo" class="alchemize-logo">`
      : "";
  const alchemizeBg =
    program.name === "Alchemize"
      ? `<img src="./logos/alchemize.png" alt="Alchemize Background" class="alchemize-bg">`
      : "";

  const shipyardAssets =
    program.name === "Shipyard"
      ? `
  ...
  const hackanomousLogo =
    program.name == "Hackanomous"
      ? `<img src="https://cdn.hackclub.com/019d9ecf-46ed-734c-b351-f9c2438d15bf/hackanomous_banner_360p.png" alt="Hackanomous Logo" class="hackanomous-logo">`
      : "";
  const hackanomousMascot =
    program.name == "Hackanomous"
      ? `<img src="https://cdn.hackclub.com/019d9ef5-f609-7d16-971f-3865d2092604/backanomous_mascot_320p.png" alt="Hackanomous Mascot" class="hackanomous-mascot">`
      : "";

  const kintsugiBg =
    program.name == "Kintsugi"
      ? `<img src="logos/Kintsugi-bg.jpg" alt="Kintsugi Background" class="kintsugi-bg">`
      : "";
  const kintsugiMascot =
    program.name == "Kintsugi"
  ...
  const displayDescription =
    program.name === "Pixl"
      ? program.description
          .replace(
            "open world",
            '<span class="pixl-highlight">open world</span>',
          )
          .replace(
            "sidequests",
            '<span class="pixl-highlight">sidequests</span>',
          )
          .replace(
            "niche prizes",
            '<span class="pixl-highlight">niche prizes</span>',
          )
      : program.description;
...
function openModal(program, originCard) {
  ...
  modal.querySelector(".program-status").className =
...
function getLeaderboardProgram(programName, shipCount) {
  ...
  const fallbackDescriptions = {
    Daydream:
      "Daydream is featured in the YSWS ship rankings, but its full catalog details are not available in the current local dataset.",
    "Campfire Satellites":
      "Campfire Satellites is featured in the YSWS ship rankings, but its full catalog details are not available in the current local dataset.",
  };

  const description =
    fallbackDescriptions[programName] ||
    "This leaderboard entry is featured in the ship rankings.";
...
function updateShowMoreButton(section, expanded) {
  ...
  const hiddenCount = Math.max(
    0,
    cards.length - getCollapsedVisibleCount(section),
  );
...
function toggleCollapsibleSection(section) {
  if (
    section.classList.contains("collapsible-disabled") ||
...
function refreshCollapsibleSections() {
  requestAnimationFrame(() => {
    document
      .querySelectorAll('.category-section[data-collapsible="true"]')
      .forEach(updateCollapsibleSection);
  });
}
...
function renderPrograms() {
  ...
  const expandedCategories = new Set(
    [
      ...container.querySelectorAll(
        ".category-section.expanded[data-category]",
      ),
  ...
  if (currentSort === "default") {
    for (const [category, programsList] of Object.entries(programs)) {
      ...
      const cardsHtml = programsList
        .map((program) => createProgramCard(program))
        .join("");

      const hiddenCount = Math.max(
        0,
        programsList.length - COLLAPSED_VISIBLE_ROWS * 3,
      );
...
function loadTimelineBlocks() {
  ...
  while (cursor <= furthestEvent) {
    ...
    const label = jan
      ? `${monthStart.toLocaleString("default", { month: "short" })} '${yearShort}`
  ...
  for (
    let i = 0;
    i <
    Math.ceil((furthestEvent.getTime() - now.getTime()) / 1000 / 60 / 60 / 24);
    i++
  ) {
  ...
  const timelineDays = Math.ceil(
    (furthestEvent.getTime() - now.getTime()) / 1000 / 60 / 60 / 24,
  );
  document.getElementById("timeline-overlay").style.width =
    `${timelineDays}rem`;
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (event.status !== "ended" && event.status !== "draft") {
      ...
      if (event.deadline) {
        days = Math.max(
          Math.ceil((event.deadline - now) / 1000 / 60 / 60 / 24),
          1,
        );
...
function updateDeadlines() {
  ...
  deadlineElements.forEach((element) => {
    ...
    if (programData?.deadline) {
      if (
        isEventEnded(programData.deadline) &&
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("programs-container")
  ...
  document.addEventListener("keydown", (e) => {
    if (!document.getElementById("program-modal").classList.contains("active"))
      return;
...
async function loadGlobalStats() {
  try {
    ...
    const localParticipantTotal = Object.values(programs)
      .flat()
      .reduce(
        (total, program) => total + (Number(program.participants) || 0),
        0,
      );
...
function getWheelPrograms() {
  ...
  const active = Object.values(programs)
    .flat()
